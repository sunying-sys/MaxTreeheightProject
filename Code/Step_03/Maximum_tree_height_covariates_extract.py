...
print(forestAgeData.bandNames().getInfo())
...
CHELSA_vpd = ee.Image("projects/ee-sun520ying/assets/CHELSA_vpd_mean_1981-2010_V_2_1").rename('CHELSA_vpd').reproject(stdProj)
waterAvail = compositeToUseRaw.select("CHELSA_Annual_Precipitation").subtract(compositeToUseRaw.select("PET")).rename("WaterAvailability").reproject(stdProj)
ET_Ver3 = ee.Image('users/leonidmoore/Evapotranspiration_0_Ver_3_2023').rename('ET0').reproject(stdProj)
intactForestMap = ee.Image("users/leonidmoore/ForestBiomass/Intact_forest_map_1km").rename('IntactForest').reproject(stdProj)
canopyHeight_GDEI = ee.Image("users/leonidmoore/TreeStructureProject/Lang_et_al_Canopy_top_height").rename('IntactForest').reproject(stdProj)
# add the bands to the composite
compositeToUse = compositeToUseRaw.addBands(soilmoisture).addBands(forestAge).addBands(ET_Ver3).addBands(waterAvail).addBands(CHELSA_vpd).addBands(intactForestMap).addBands(canopyHeight_GDEI)
...
points = ee.FeatureCollection("projects/ee-sun520ying/assets/Pixel_Level_tree_height_max_20260407_filtered")
